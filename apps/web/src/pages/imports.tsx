...
export function ImportsPage() {
  ...
  return (
    <div className="space-y-4">
      <PageHeader title={t('imports.title')} subtitle={t('imports.subtitle')} actions={<Badge>{imports.data?.length ?? 0}</Badge>} />
      <Card className="space-y-4">
        ...
        {previewMutation.error && <Alert tone="error">{previewMutation.error.message}</Alert>}
        {importMutation.error && <Alert tone="error">{importMutation.error.message}</Alert>}
      ...
      <Card>
        ...
        <Table
          ...
          rows={(imports.data ?? []).map((item) => [item.fileName, <StatusBadge key={item.id} status={item.status} />, item.rowCount, item.validRows, item.invalidRows, new Date(item.createdAt).toLocaleString(locale)])}
        />
      </Card>
    </div>
  );
